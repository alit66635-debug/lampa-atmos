(function () {
    ...
    /* ═══════════════════════════════════════════════════════════
       ...
       v1.4: Atmos из описания релиза с привязкой к языку дорожки.
       ═══════════════════════════════════════════════════════════ */

    var VERSION     = '1.4-final';
    var DEBUG       = false;
    ...
    var CACHE_KEY   = 'mediainfo_cache_v3';
    ...
    function log(m) {
        if (DEBUG) {
            try { console.log('[MediaInfo] ' + m); } catch (e) {}
        }
    }
    ...
    function loadCache() {
        try {
            Lampa.Storage.set('mediainfo_cache_v1', {});
        } catch (e) {}
    ...
    function extractHash(s) {
        ...
        var b = s.match(/urn:btih:([A-Za-z2-7]{32})/);
        if (b) return b[1];

        var h = s.match(/\b([a-fA-F0-9]{40})\b/);
    ...
    function parseTitle(title) {
        ...
        var codec =
            /hevc|h\.?265|x265/.test(s) ? 'HEVC' :
            /av1/.test(s) ? 'AV1' :
            /h\.?264|x264|\bavc\b/.test(s) ? 'H.264' :
            '';

        var hdr =
            /dolby\s*vision|dovi|dvhe|dv\s*p[78]/.test(s) ? 'Dolby Vision' :
    ...
    function cleanCodec(c) {
        ...
        return /^[A-Za-z0-9][A-Za-z0-9.+_-]{0,11}$/.test(c)
            ? c.toUpperCase().replace('H264', 'H.264')
            : '';
    }

    function resLabel(v) {
        var w = v.width || v.coded_width || 0;
        var h = v.height || v.coded_height || 0;
    ...
    function hdrLabel(v) {
        ...
        try {
            dv = (v.side_data_list || []).some(function (sd) {
                return /dovi|dolby vision/i.test(
                    JSON.stringify(sd)
                );
            });
        } catch (e) {}

        if (!dv && /dvhe|dvh1|dav1|dvav/i.test(
            v.codec_tag_string || ''
        )) {
            dv = true;
        }
    ...
    function textFields(root) {
        ...
        function walk(o, d) {
            ...
            Object.keys(o).forEach(function (k) {
                ...
                if (
                    /^(description|desc|release|releaseinfo|release_info|comment|annotation)$/.test(lk)
                ) {
                    add(v);
                }
        ...
        function walk(o, d) {
            ...
            Object.keys(o).some(function (k) {
                ...
                if (
                    typeof v === 'string' &&
                    /^https?:\/\//i.test(v) &&
                    /^(details?|details?_url|details?_link|page|page_url|desc_link|guid|tracker_url)$/.test(lk)
                ) {
                    found = v;
                    return true;
                }
    ...
    function languageAliases(lang) {
        ...
        var names = LANG_NAMES[code] || [code];
    ...
    /*
       Ищем Atmos внутри блока конкретной дорожки:
       от найденного языка до следующего языка.
       ...
       Atmos будет только у ENG.
    */

    function releaseAtmosForLanguage(text, lang) {
        text = normText(text).toLowerCase();

        if (!text) return false;

        var aliases = languageAliases(lang);
        ...
        Object.keys(LANG_NAMES).forEach(function (k) {
            languageAliases(k).forEach(function (a) {
                if (allAliases.indexOf(a) < 0) {
                    allAliases.push(a);
                }
            });
        });

        var targetRx = new RegExp(
            '(?:' + aliases.join('|') + ')',
            'gi'
        );

        var nextRx = new RegExp(
            '(?:' + allAliases.join('|') + ')',
            'gi'
        );
        ...
        while ((m = targetRx.exec(text))) {
            var start = m.index + m[0].length;

            nextRx.lastIndex = start;

            var n = nextRx.exec(text);
            ...
            if (
                /\batmos\b|dolby\s*atmos|\bjoc\b/i.test(block)
            ) {
                return true;
            }
        }
    ...
    function releaseAtmos(text, streams) {
        var result = {};
        ...
        aud.forEach(function (a) {
            ...
            if (
                l &&
                releaseAtmosForLanguage(text, l)
            ) {
                result[l] = true;
            }
        });
        ...
        /*
           Если Atmos есть в описании, но язык в описании
           отсутствует — fallback только при единственной
           E-AC3-дорожке.
        */

        if (
            !Object.keys(result).length &&
            /\batmos\b|dolby\s*atmos|\bjoc\b/i.test(
                normText(text)
            )
        ) {
            var e = aud.filter(function (a) {
                return (
                    /eac3|ec-3|e-ac-3/i.test(
                        a.codec_name || ''
                    ) ||
                    /eac3|e-ac-3|dd\+|dolby digital plus/i.test(
                        (
                            a.tags &&
                            (a.tags.title || '')
                        ) || ''
    ...
    function chTxt(a) {
        ...
        if (a.channels === 2) return '2.0';
        if (a.channels === 1) return '1.0';
    ...
    function build(streams, releaseText) {
        ...
        if (v) {
            ...
            var cc = cleanCodec(v.codec_name);
            if (cc) p.push(cc);
        ...
        streams
            ...
            .forEach(function (a) {
                ...
                if (
                    nm &&
                    !/SoundHandler|AudioHandler/i.test(nm)
                ) {
                    nm = String(nm)
                        .replace(/[<>]/g, '');
    ...
    function mergeRows(
        ...
        if (!v) {
            var c = titleVideoChip(pt);

            if (c) rows.unshift(c);
        } else {
            if (
                pt.res &&
                !/4K|1080p|720p|480p|\d{3,4}x\d{3,4}/.test(v.t)
            ) {
                v.t =
                    pt.res +
                    ' · ' +
        ...
        nativeReq(
            ...
            function (jq, ex) {
                ...
                if (st === 400) {
                    ...
                    log(
                        'ffp 400 → нет ffprobe, далее только 185'
                    );
    ...
    function onTorrentRender(
        element,
        item
    ) {
        if (
            element.ffprobe &&
            element.ffprobe.length
        ) {
            renderRows(
                item,
                safeRows(
    ...
    function start() {
        ...
        st.textContent =
            ...
            '.mi-badge span{border-radius:.3em;padding:.05em .45em;white-space:nowrap;border:1px solid;max-width:100%;box-sizing:border-box}' +
            '.mi-badge .mi-a{white-space:normal;overflow-wrap:anywhere;word-break:break-word}' +
